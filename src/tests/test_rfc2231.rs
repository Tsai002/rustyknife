use crate::rfc2231::*;
use crate::rfc2231::{ContentDisposition as CD, ContentTransferEncoding as CTE};

#[cfg_attr(not(feature = "quoted-string-rfc2047"), should_panic)]
#[test]
fn rfc2047() {
    let (rem, (mtype, params)) = content_type(
        b" message/external-body; name=\"a =?utf-8?b?w6l0w6kgYmxvcXXDqQ==?= par ZEROSPAM.eml\"",
    )
    .unwrap();
    assert_eq!(rem.len(), 0);
    ...
    assert_eq!(
        params,
        [("name".into(), "a été bloqué par ZEROSPAM.eml".into())]
    );
}
...
fn header_lf() {
    ...
    assert_eq!(
        ...
        [(
            "name".into(),
            "Fiche d’information relative à la garantie facultative.pdf".into()
        )]
    );
}
...
fn attmsg1() {
    ...
    assert_eq!(
        ...
        [(
            "name".into(),
            "[The Listserve] Have you ever seen something you couldn’t explain?.eml".into()
        )]
    );
}
...
fn attmsg3() {
    ...
    assert_eq!(
        ...
        [(
            "name".into(),
            "[decoupe CNC] Re: H_S_ envoyer de « gros » fichiers.eml".into()
        )]
    );
}
...
fn simple_long() {
    ...
    assert_eq!(
        params,
        [
            ("access-type".into(), "URL".into()),
            (
                "url".into(),
                "ftp://cs.utk.edu/pub/moore/bulk-mailer/bulk-mailer.tar".into()
            )
        ]
    );
}
...
fn encoded_mixed() {
    ...
    assert_eq!(
        ...
        [(
            "title".into(),
            "This is even more ***fun*** isn't it!".into()
        )]
    );
}
...
green_tc!(
    attfnboth,
    b"attachment; filename=\"foo-ae.html\"; filename*=UTF-8''foo-%c3%a4.html;",
    CD::Attachment,
    "foo-ä.html"
);
green_tc!(
    attfnboth2,
    b"attachment; filename*=UTF-8''foo-%c3%a4.html; filename=\"foo-ae.html\"",
    CD::Attachment,
    "foo-ä.html"
);
...
green_tc!(
    attfncontenc,
    b"attachment; filename*0*=UTF-8''foo-%c3%a4; filename*1=\".html\"",
    CD::Attachment,
    "foo-ä.html"
);
green_tc!(
    attfncontord,
    b"attachment; filename*1=\"bar\"; filename*0=\"foo\"",
    CD::Attachment,
    "foobar"
);
green_tc!(
    attwithasciifnescapedchar,
    b"inline; filename=\"f\\oo.html\"",
    CD::Inline,
    "foo.html"
);
green_tc!(
    attwithfn2231abspathdisguised,
    b"attachment; filename*=UTF-8''%5cfoo.html",
    CD::Attachment,
    "\\foo.html"
);
green_tc!(
    attwithfn2231utf8,
    b"attachment; filename*=UTF-8''foo-%c3%a4-%e2%82%ac.html",
    CD::Attachment,
    "foo-ä-€.html"
);
green_tc!(
    attwithfntokensq,
    b"attachment; filename='foo.bar'",
    CD::Attachment,
    "'foo.bar'"
);
green_tc!(
    attwithisofnplain,
    b"attachment; filename=\"foo-\xe4.html\"",
    CD::Attachment,
    "foo-\u{fffd}.html"
);
green_tc!(
